...
#define NODE_PRINTOUT_DELAY 1000 //Delay between printing node values 
...
#define DEFAULT_SLEEP_PERIOD_SENSOR3 3
#define DEFAULT_SLEEP_PERIOD_SENSOR2 1
#define DEFAULT_SLEEP_PERIOD_SENSOR1 2
...
cout << "0" << ltm->tm_min << ":";
  ...
  // Setup and configure rf radio
	...
	while (1)
	{
		...
		if(millis()-last_seenSensor2 >= (SLEEP_PERIOD_SENSOR2*1*1000) || node2_lost){
 		node2_lost = retryFetchSensor(2, 8, 0.25);
		}
		
		if(millis()-last_seenSensor1 >= (SLEEP_PERIOD_SENSOR1*2*1000) || node1_lost){
